...
class ProductController extends Controller
{
    ...
    public function viewProduct(Request $request, $id = null)
    {
        ...
        /** TBI Bank */
        $tbi_unicid = "1af4d77b-70a2-4771-a04b-1c6d63f07680";
        $product_id = $product->id;
        $tierPrice = floatval($product->price);
        //get parameters
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_URL, 'https://tbibank.support/function/getparameters.php?cid=' . $tbi_unicid);
            ...
            if (is_numeric($product_id)) {
                ...
                if (in_array($product_category, $categories)) {
                    ...
                    if (in_array($manufacturer_id, $manufacturers)) {
                        ...
                        if ((doubleval($paramstbi['tbi_4m_min']) <= $tierPrice) && ($tierPrice <= doubleval($paramstbi['tbi_4m_max']))) {
                            $is4m = 'Yes';
                        } else {
                            $is4m = 'No';
                        }
            ...
            if (is_numeric($product_id)) {
                ...
                if (in_array($product_category, $categories)) {
                    ...
                    if (in_array($manufacturer_id, $manufacturers)) {
                        ...
                        if ((doubleval($paramstbi['tbi_5m_min']) <= $tierPrice) && ($tierPrice <= doubleval($paramstbi['tbi_5m_max']))) {
                            $is5m = 'Yes';
                        } else {
                            $is5m = 'No';
                        }
            ...
            if (is_numeric($product_id)) {
                ...
                if (in_array($product_category, $categories)) {
                    ...
                    if (in_array($manufacturer_id, $manufacturers)) {
                        ...
                        if ((doubleval($paramstbi['tbi_6m_min']) <= $tierPrice) && ($tierPrice <= doubleval($paramstbi['tbi_6m_max']))) {
                            $is6m = 'Yes';
                        } else {
                            $is6m = 'No';
                        }
            ...
            if (is_numeric($product_id)) {
                ...
                if (in_array($product_category, $categories)) {
                    ...
                    if (in_array($manufacturer_id, $manufacturers)) {
                        ...
                        if ((doubleval($paramstbi['tbi_9m_min']) <= $tierPrice) && ($tierPrice <= doubleval($paramstbi['tbi_9m_max']))) {
                            $is9m = 'Yes';
                        } else {
                            $is9m = 'No';
                        }
		...
		// схема 10+1
		if (($paramstbi['tbi_purcent_default'] == 11) || ((($paramstbi['tbi_purcent_default'] == 2) && (($is4m == "Yes") || ($is4m_pv == "Yes"))) || (($paramstbi['tbi_purcent_default'] == 3) && (($is6m == "Yes") || ($is6m_pv == "Yes")) ) || ( ($paramstbi['tbi_purcent_default'] == 5) && (($is9m == "Yes") || ($is9m_pv == "Yes")) ) )){
			$oskapiavane_12 = 0.015;		
			$vnoska = $tierPrice / $tpurcent;
		}else{
			if ($paramstbi['tbi_purcent_default'] == 14){
				$oskapiavane_12 = 0.0198;		
				$vnoska = (($tierPrice - ($tierPrice * 0.10)) * (1 + $oskapiavane_12 * 12)) / 12;					
			}else{
				...
				$vnoska = ($tierPrice * (1 + $oskapiavane_12 * $tpurcent)) / $tpurcent;
			}
		}
		// схема 10+1
		// shema 101
		...
		if ($paramstbi['tbi_taksa_categories'] != ""){
            ...
            if (in_array($product_category, $cats)){
                $isTaksa = 'Yes';
            }else{
                $isTaksa = 'No';
            }		
